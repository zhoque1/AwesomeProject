...
const getAppToken = async () => {
  let credentials = {accessToken: 'Hello'}; // getAuthState()
...
instance.interceptors.request.use(res => {
  return Config.API_LOG ? AxiosLogger.requestLogger(res) : res;
});

instance.interceptors.response.use(res => {
  return Config.API_LOG ? AxiosLogger.responseLogger(res) : res;
});
